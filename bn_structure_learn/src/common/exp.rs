...
struct ExpSetting {
    pub method: String,
    pub bdeu_ess: Vec<f64>, // 変更: f64 から Vec<f64> へ
    pub valid_rate: f64,
    pub data_dir: String,
    ...
    pub timeout_valid: bool, // タイムアウト有効/無効のフラグを追加
}
...
pub fn exp_order() {
    ...
    for data_name in exp_setting.data_names {
        for ess in &exp_setting.bdeu_ess { // 各 bdeu_ess に対してループ
            let save_dir = format!("{}/{}/ess_{}", exp_setting.saving_dir, data_name, ess);
            // もしディレクトリが存在しないなら作成
            ...
            if *running.0.lock().unwrap() && start_time.elapsed() < timeout_duration {
                ...
                writeln!(log_file, "Experiment for {} with ess {} completed successfully.", data_name, ess).expect("Failed to write to log file");
                ...
                *run = false;
                ...
                if !*running.0.lock().unwrap() {
                    let mut log_file = OpenOptions::new().append(true).create(true).open(&log_file_path).expect("Cannot open log file");
                    writeln!(log_file, "Experiment for {} with ess {} was stopped after reaching the timeout limit.", data_name, ess).expect("Failed to write to log file");
